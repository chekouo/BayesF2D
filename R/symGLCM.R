...
#' @param Gam list of 36 4x4 correlation matrices
#' @param noise noise
...
symGLCM = function(m, # list of 4 2x1 means bivariate indicating the highest intensity coordinates in GLCMs
                   S, # list of 4 2x2 covariance matrices of the bivariate normal
                   Gam, # list of 36 4x4 correlation matrices
                   noise, # noise
  ...
  eps = lapply(1:(g_size*(g_size+1)/2), function(l) MASS::mvrnorm(1, mu = rep(0, 4), Sigma = noise*Gam[[l]]))
  ...
  listGLCM = lapply(1:4,
                    function(r){
                      ...
                      l = 0
                      for(k in 1:g_size) for(j in k:g_size){
                        l = l+1
                        ...
                        a4 = mvtnorm::pmvnorm(upper = c(k-1,g_size-j),
                                              mean = m[[r]], sigma = S[[r]])
                        p_jk = a1-a2-a3+a4
                        if(j==k) temp_term[j,k] = log(p_jk)+eps[[l]][r] 
                        if(j!=k) temp_term[j,k] = temp_term[k,j] = log(p_jk)+eps[[l]][r]
                      }
